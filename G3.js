...
G3.fn.extend({
    ...
    loginMap: function (url, type, data) {
        ...
        for (let i in this.opt) {
            ...
            //当两者都是数组时
            else if (this.opt[i] instanceof Array === true && this.dataOpt[i] instanceof Array === true) {
                //当数组第一位为数组时候
                var interimArray = [];
                //console.log(this.opt[i][this.opt[i].length - 1])
                if (this.opt[i][0] instanceof Array) {
                    //console.log(this.opt[i][0]);
                    for (let k = 0; k < this.opt[i][0].length; k++) {
                        //console.log(this.opt[i][0][k]);
                        interimArray.push(this.opt[i][0][k]);
                    }
                    //console.log(interimArray)
                    if (this.opt[i][this.opt[i].length - 1] == 'null' || this.opt[i][this.opt[i].length - 1] === null) {
                        interimArray.push(null)
                    }
                    this.opt[i] = interimArray;
                    interimArray = null;
                }
            ...
            //自身是对象属性，模板是数组时
            else if (this.opt[i] instanceof Object === true && this.dataOpt[i] instanceof Array === true) {
            ...
            //两者都是对象时候，进行内部遍历替换
            else if (this.opt[i] instanceof Object && this.dataOpt[i] instanceof Object) {
                //替换已经存在的
                for (let j in this.opt[i]) {
                    this.dataOpt[i][j] = this.opt[i][j];
                    //console.log(i, this.opt[i][j], typeof this.opt[i][j]);
                }
            } else {
                this.dataOpt[i] = this.opt[i];
            }
     ...
     * 如果调用getData，可免去调用
     * */
    drawChart: function (opt,notMerge,lazyUpdate) {
        if (!this.xmlhttp) {
            this.opt = opt;
            this.myChart.setOption(this.opt,notMerge,lazyUpdate);
        } else {
            this.myChart.setOption(this.formatChart(opt, this.xmlhttp),notMerge,lazyUpdate);
        }
        return this;
    },
